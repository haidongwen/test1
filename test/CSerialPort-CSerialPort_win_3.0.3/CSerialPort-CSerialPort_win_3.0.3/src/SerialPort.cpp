...
void CSerialPort::ReceiveStr(CSerialPort* port)
{
	...
	for (;;)
	{
		...
		//如果遇到'\r'，那么数据会被截断，实际数据全部读取只是没有显示完全，这个时候使用memcpy才能全部获取
		unsigned char* RXBuff = new unsigned char[comstat.cbInQue + 1];
		if (RXBuff == NULL)
		{
			return;
		}
		RXBuff[comstat.cbInQue] = '\r';//附加字符串结束符
		//RXBuff[comstat.cbInQue] = '*';//附加字符串结束符

		EnterCriticalSection(&port->m_csCommunicationSync);
		if (bRead)
		{
			///串口读出，读出缓冲区中字节
			bResult = ReadFile(port->m_hComm,		// Handle to COMM port 文件句柄
				RXBuff,				// RX Buffer Pointer 缓冲区
				comstat.cbInQue,					// Read cbInQue len byte 要读入的字节数  comstat.cbInQue
				&BytesRead,			// Stores number of bytes read 实际读取字节的指针
				&port->m_ov);		// pointer to the m_ov structure
			 /*deal with the error code 
			若返回错误，错误处理*/
			if (!bResult)
			{
				...
				case ERROR_IO_PENDING:
				{
					...
					// Proceed on to GetOverlappedResults();
					///异步IO仍在进行
					WaitForSingleObject(port->m_hComm,4000);
					bRead = FALSE;
					break;
				}
				case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
